...
def getOptimalDirectoryToDelete(rootDirectory: Directory):
  directorySizes = getFileTreeDirectorySizes(rootDirectory) 
  availableFreeSpace = TOTAL_DISK_SPACE - rootDirectory.getSize()
  neededFreeSpace = UPDATE_SIZE - availableFreeSpace 

  potentialDirectories = list(filter(lambda size: size > neededFreeSpace, directorySizes))
  potentialDirectories.sort()

  return potentialDirectories[0]

rootDirectory = buildFileTree(open('input.txt').readlines())
print(getOptimalDirectoryToDelete(rootDirectory))